    ...
    static bool triangles(engine& __rt, std::ifstream& __ifile) {
        ...
        for (register unsigned short __i = 0; __i < 6; __i++) {
            ...
            if (__slinel == "vertices") {
                ...
                __material._color[3] = 0.f; // opaque
            } else {
...
*/
        // transformations
        transformation __last2;
        _transformations.top(__last2);

        // new triangle
        matrix<4, 4> __temp2 = __last2.t();
	 polygon *__polygon = new polygon(__temp2, __material, points , 4);

        // add object
        //__rt.add_object((object*) __triangle);
        __rt.add_object((object*) __polygon);
